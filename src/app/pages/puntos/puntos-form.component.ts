...
import { PuntoService }       from '../../core/services/domain/punto.service';
...
export class PuntosForm implements OnInit {
  private readonly auth   = inject(AuthService);
  private readonly svc    = inject(PuntoService);
  ...
  readonly tipoOpts = ['PORCENTAJE','FIJO','MIXTO'];
      ...
      this.svc.getById(+id).subscribe({
        next: (r: any) => {
          this.form.patchValue({
            ...
            tipo:         r.tipo ?? 'FIJO',
            puntos:       r.puntos ?? 0,
            importeDesde: r.importeDesde ?? 0,
            importeHasta: r.importeHasta ?? 999999999,
  ...
  guardar(): void {
    ...
    op.subscribe({
      next: () => { this.guardando.set(false); this.toast.success(id ? 'Punto actualizado' : 'Punto creado'); this.router.navigate(['/puntos']); },
