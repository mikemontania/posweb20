...
export class MotivoTransferenciaComponent implements OnInit {
  ...
  items   = signal<MotivoTransferencia[]>([]);
  loading = signal(false);
  termino = '';

  showDelModal  = signal(false);
  itemAEliminar = signal<MotivoTransferencia | null>(null);
  eliminando    = signal(false);

  onSearch(t: string): void { this.termino = t; }

  get itemsFiltrados(): MotivoTransferencia[] {
  ...
  cargar(): void {
    ...
    this.svc.getAll({ codempresa: codEmp }).subscribe({
      ...
      error: () => { this.loading.set(false); this.toast.error('Error al cargar Motivos de Transferencia'); }
    });
  }

  solicitarEliminar(item: MotivoTransferencia): void {
  ...
  confirmarEliminar(): void {
    ...
    this.svc.delete((item as any).codMotivoTransferencia).subscribe({
      next: () => { this.showDelModal.set(false); this.eliminando.set(false);
        this.toast.success('Motivos de Transferencia eliminado'); this.cargar(); },
