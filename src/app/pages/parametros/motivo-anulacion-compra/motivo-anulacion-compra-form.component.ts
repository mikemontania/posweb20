...
@Component({
  selector: 'app-motivo-anulacion-compra-form',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './motivo-anulacion-compra-form.component.html',
  styleUrl: './motivo-anulacion-compra.component.css',
})
export class MotivoAnulacionCompraFormComponent implements OnInit {
  ...
  item      = signal<MotivoAnulacionCompra>({} as MotivoAnulacionCompra);
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.item.set({ codEmpresa: codEmp } as MotivoAnulacionCompra);
    }
  ...
  guardar(): void {
    ...
    const op = this.esEdicion()
      ? this.svc.update((data as any).codMotivoAnulacionCompra, data)
      : this.svc.create(data);
    op.subscribe({
      next: () => {
        this.guardando.set(false);
        this.toast.success(this.esEdicion() ? 'Actualizado' : 'Creado');
        this.router.navigate(['/motivo-anulacion-compra']);
      },
