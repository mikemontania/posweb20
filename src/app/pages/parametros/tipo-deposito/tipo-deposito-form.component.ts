...
@Component({
  selector: 'app-tipo-deposito-form',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './tipo-deposito-form.component.html',
  styleUrl: './tipo-deposito.component.css',
})
export class TipoDepositoFormComponent implements OnInit {
  ...
  item      = signal<TipoDeposito>({} as TipoDeposito);
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.item.set({ codEmpresa: codEmp } as TipoDeposito);
    }
  ...
  guardar(): void {
    ...
    const op = this.esEdicion()
      ? this.svc.update((data as any).codTipoDeposito, data)
      : this.svc.create(data);
    op.subscribe({
      next: () => {
        this.guardando.set(false);
        this.toast.success(this.esEdicion() ? 'Actualizado' : 'Creado');
        this.router.navigate(['/tipo-deposito']);
      },
