...
export class GrupoDescuentoFormComponent implements OnInit {
  ...
  item      = signal<GrupoDescuento>({} as GrupoDescuento);
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.item.set({ codEmpresa: codEmp } as GrupoDescuento);
    }
  ...
  guardar(): void {
    ...
    op.subscribe({
      next: () => {
        ...
        this.router.navigate(['/grupo-descuento']);
      },
