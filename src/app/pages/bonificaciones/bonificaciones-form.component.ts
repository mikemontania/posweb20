...
@Component({
  selector: 'app-bonificaciones-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterModule, SelectSearchComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './bonificaciones-form.component.html',
  styleUrl: './bonificaciones.component.css',
})
export class BonificacionesForm implements OnInit {
  private readonly auth   = inject(AuthService);
  private readonly svc    = inject(BonificacionService);
  ...
  selMaterialBonif = signal<any>(null);

  form = this.fb.group({
    descripcion:      ['', Validators.required],
    tipoBonificacion: ['CANTIDAD'],
    cantDesde:        [1],
    cantHasta:        [999999999],
    cantBonif:        [0],
    grpMaterial:      [''],
      ...
      this.svc.getById(+id).subscribe({
        next: (r: any) => {
          this.form.patchValue({
            ...
            tipoBonificacion: r.tipoBonificacion ?? 'CANTIDAD',
            cantDesde:        r.cantDesde ?? 1,
            cantHasta:        r.cantHasta ?? 999999999,
            cantBonif:        r.cantBonif ?? 0,
            grpMaterial:      r.grpMaterial ?? '',
          ...
          this.selMaterialBonif.set(r.materialBonif ?? null);
  ...
  guardar(): void {
    ...
    const payload: any = {
      ...
      materialBonif: this.selMaterialBonif(),
      codSucursal:  this.selSucursal()?.codSucursal ?? null,
    };
    const id = this.route.snapshot.paramMap.get('id');
    const op = id ? this.svc.update(+id, payload) : this.svc.create(payload);
    op.subscribe({
      next: () => { this.guardando.set(false); this.toast.success(id ? 'Bonificación actualizada' : 'Bonificación creada'); this.router.navigate(['/bonificaciones']); },
