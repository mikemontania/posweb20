...
export class ClientesComponent implements OnInit {
    ...
    this.svc.getPage(page, this.termino(), codEmp).subscribe({
      next: (r: any) => {
        ...
        const mapped = raw.map((c: any) => ({
          ...c,
          _listaPrecio: c.listaPrecio?.descripcion ?? '—',
          _sucursal:    c.sucursalPref?.nombreSucursal ?? '—',
          _medioPago:   c.medioPagoPref?.descripcion ?? '—',
          _formaVenta:  c.formaVentaPref?.descripcion ?? '—',
        }));
  ...
  solicitarEliminar(item: any): void {
  ...
  confirmarEliminar(): void {
    ...
    this.svc.delete(item.codCliente).subscribe({
      next: () => {
